...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 4. 실패 시 서버 에러 메시지 표시
      const serverError = err.response?.data?.detail?.error || "회원가입 중 오류가 발생했습니다.";
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6">
      ...
      <div className="w-full max-w-[480px] bg-white rounded-[3rem] p-8 md:p-12 border border-black/5 shadow-2xl shadow-black/5 animate-in fade-in slide-in-from-bottom-8 duration-1000">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">회원가입</h1>
          <p className="text-slate-500 font-medium mt-2">지식의 여정을 함께 시작해 보세요.</p>
        </div>
